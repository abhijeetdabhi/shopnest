...
if (isset($_COOKIE['latitude']) && isset($_COOKIE['longitude'])) {
    function getAddressFromLatLng($lat, $lng, $apiKey)
    {
        ...
        if (isset($data['addresses'][0]['address'])) {
            ...
            $street = isset($address['street']) ? $address['street'] : 'Not Available';
            $municipalitySubdivision = isset($address['municipalitySubdivision']) ? $address['municipalitySubdivision'] : 'Not Available';
    ...
    if ($address) {
        ...
        $fullAddress = $street . ', ' . $municipalitySubdivision . ', ' . $usercity;
    }
} else {
    $street = '';
...
<body style="font-family: 'Outfit', sans-serif;" x-data="{ logoutPopUp: false }" :class="{ 'overflow-hidden ': logoutPopUp}">
    ...
    <div id="loader" class="flex-col gap-4 w-full flex items-center justify-center bg-black/30 fixed top-0 h-full backdrop-blur-sm z-40" style="display: none;">
        ...
        <img class="w-10 absolute" src="/shopnest/shopnest/src/logo/black_cart_logo.svg" alt="Cart Logo">
    </div>
    <script>
        function loader() {
        ...
        ?>
            <div id="topBar" class="flex justify-between items-center space-x-2 p-2 shadow-inner shadow-black/70">
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <a href="/shopnest/pages/map.php" class="flex flex-col cursor-pointer">
        ...
        <header class="bg-black px-2 py-4 outfit relative">
            <div class="flex items-center justify-between gap-10">
                <div class="flex xl:gap-5">
                    ...
                    <!-- logo -->
                    <div>
                        <a class="flex w-fit py-2 focus:outline-none logoAnimation" href="/shopnest/index.php">
                            <!-- icon logo div -->
                            <div>
                                <img class="w-8 sm:w-12" src="/shopnest/src/logo/white_cart_logo.svg" alt="">
                            </div>
                            <!-- text logo -->
                            <div>
                                <img class="w-20 sm:w-32" src="/shopnest/src/logo/white_text_logo.svg" alt="">
                            </div>
                        </a>
                    </div>
                </div>
                ...
                <div class="relative h-full hidden lg:block" x-data="{showMic:false}">
                    ...
                    ?>
                        <script>
                            ...
                            if (filterName != "") {
                                window.location.href = "/shopnest/search/search_items.php?searchName=" + filterName;
                            }
                        </script>
                    <?php
                    }
                    ...
                    ?>
                        ...
                        <div x-data="{loginUser:false}" class="relative">
                            ...
                            <div x-show="loginUser" x-transition x-cloak class="w-40 bg-white text-black overflow-hidden absolute -left-14 top-10 md:top-12 shadow-lg shadow-black/60 rounded-md z-10">
                                <div class="py-2 px-3">
                                    <a href="/shopnest/index.php" class="w-full">Hi,<?php echo isset($_COOKIE['fname']) ? $_COOKIE['fname'] : 'User Name' ?></a>
                                </div>
                                <hr class="border">
    ...
    <script>
        function showSidebar() {
            ...
            activePopup = 'main-sidebarContainer';
            let mainSidebarContainer = $('#main-sidebarContainer');
            mainSidebarContainer.show();
        ...
        $(document).keydown(function(event) {
            if (event.key === 'Escape') {
                if (activePopup === 'main-sidebarContainer') {
                    closeSidebar();
                }
            }
        });
        ...
        function closeSidebar() {
            ...
            setTimeout(function() {
                ...
                let mainSidebarContainer = $('#main-sidebarContainer').hide();
            }, 300);
            // $('body').fadeTo(800,1);   
        }
    </script>
