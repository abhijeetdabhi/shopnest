...
if (isset($_POST['submitBtn'])) {
    ...
    if (move_uploaded_file($tempname, $folder) && move_uploaded_file($tempname2, $folder2) && $lat != "" || $lat != "" && !preg_match($name_pattern, $name) || !preg_match($email_pattern, $email) || !preg_match($password_pattern, $password) || !preg_match($username_pattern, $username) || !preg_match($phone_pattern, $phone) || !preg_match($gst_pattern, $gst) || !preg_match($bio_pattern, $bio)) {
        ...
        $insert_data = "INSERT INTO vendor_request(name, email, password, username, phone, Bio, GST, cover_image, dp_image, latitude, longitude, status, date) VALUES ('$vendorName','$vendorEmail','$vendorPassword','$vendorUsername','$vendorPhone','$vendorBio','$vendorGST','$vendorCover_image','$vendorDp_image','$vendorLatitude','$vendorLongitude', 'Pending','$vendorRegiDate')";
