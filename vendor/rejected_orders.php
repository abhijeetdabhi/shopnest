...
if (isset($_COOKIE['vendor_id'])) {
    ...
    $oldData = "SELECT * FROM orders WHERE vendor_id = '$vendor_id' AND status = 'pending'";
    $oldDataQuery = mysqli_query($con, $oldData);
    $oldCount = mysqli_num_rows($oldDataQuery);
    
    $_SESSION['existingOrder'] = $oldCount;
    $existingOrder = $_SESSION['existingOrder'];
        ...
        <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0" x-cloak>
                ...
                <nav class="mt-10">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 text-gray-500 hover:bg-gray-700 bg-opacity-25 hover:text-gray-100" href="order_request.php">
                        <svg class="w-6 h-6 fill-white" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" width="512" height="512" x="0" y="0" viewBox="0 0 128 128" style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                            <g>
            ...
            <div class="flex flex-col flex-1 overflow-hidden">
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    ...
                    <section class="container mx-auto p-6">
                        <h2 class="font-manrope font-bold text-4xl leading-10 text-black mb-5">Deny Orders</h2>
                        <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
                            <div class="w-full overflow-x-auto h-max text-center">
                                <?php
                                $get_denyOrders = "SELECT * FROM deny_orders WHERE vendor_id = '$vendor_id'";
                                $get_denyOrders_query = mysqli_query($con, $get_denyOrders);

                                if (mysqli_num_rows($get_denyOrders_query) > 0) {
                                ?>
                                    <table class="w-full">
                                        <thead>
                                            <tr class="text-md font-semibold tracking-wide text-center text-gray-900 bg-gray-100 border-b border-gray-600">
                                                <th class="px-4 py-3">Deny&nbsp;order&nbsp;ID</th>
                                                <th class="px-4 py-3 w-96">Deny&nbsp;order&nbsp;Title</th>
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;Image</th>
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;Price</th>
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;Color</th>
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;Size</th>
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;QTY</th>
                                                ...
                                                <th class="px-4 py-3">Deny&nbsp;Order&nbsp;Date</th>
                                            </tr>
                                        </thead>
                                        <?php
                                    }
                                    if (isset($_COOKIE['vendor_id'])) {
                                        ...
                                        if (mysqli_num_rows($get_denyOrders_query) > 0) {
                                            while ($ro = mysqli_fetch_assoc($get_denyOrders_query)) {
                                        ?>
                                                <tbody class="bg-white border">
                                                    <tr class="text-gray-700">
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['deny_order_id'] : 'order_id' ?></td>
                                                        <td class="px-4 py-3 leading-9 line-clamp-3"><?php echo isset($_COOKIE['vendor_id']) ? $ro['order_title'] : 'order_title' ?></td>
                                                        <td class="px-3 py-2 border"><img src="<?php echo isset($_COOKIE['vendor_id']) ? '../src/product_image/product_profile/' . $ro['order_image'] : '../src/sample_images/product_1.jpg' ?>" alt="" class="w-28 h-full object-contain m-auto"></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['order_price'] : 'order_price' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['order_color'] : 'order_color' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['order_size'] : 'order_size' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['qty'] : 'order qty' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_first_name'] . ' ' . $ro['user_last_name'] : 'user_name' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_email'] : 'user_email' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_mobile'] : 'user_phone' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_address'] : 'user_address' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_state'] : 'user_state' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_city'] : 'user_city' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['user_pin'] : 'user_pin' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['payment_type'] : 'payment_type' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['Status'] : 'Status' ?></td>
                                                        <td class="px-4 py-3 border"><?php echo isset($_COOKIE['vendor_id']) ? $ro['date'] : 'date' ?></td>
                                                    </tr>
                                                </tbody>
                                            <?php
                                            }
