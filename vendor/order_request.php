        ...
        <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0" x-cloak>
                ...
                <nav class="mt-10">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 bg-gray-700 bg-opacity-25 text-gray-100" href="order_request.php">
            ...
            <div class="flex flex-col flex-1 overflow-hidden">
                ...
                <main class="relative flex-1 overflow-x-hidden overflow-y-auto scrollBar bg-gray-200">
                    <div class="py-12 max-w-screen-xl m-auto px-6">
                        <span class="text-3xl font-semibold">New Order Requests</span>
                        <div class="grid grid-cols-1 min-[480px]:grid-cols-2 md:grid-cols-3 min-[1258px]:grid-cols-4 gap-y-8 text-[#1d2128] mt-4">
                            <?php
                            if (isset($_COOKIE['vendor_id'])) {
                                $vendor_data = "SELECT * FROM orders WHERE vendor_id = '$vendor_id' AND status = 'pending'";
                                ...
                                if (mysqli_num_rows($vendor_query) > 0) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($vendor_query)) {
                                        ...
                                        <div class="flex justify-center relative">
                                            <div class="bg-white border rounded-tl-2xl rounded-br-2xl transition transform hover:shadow-lg overflow-hidden h-full w-[17rem]">
                                                <div class="p-3">
                                                    <div>
                                                        <img src="<?php echo isset($_COOKIE['vendor_id']) ? '../src/product_image/product_profile/' . $row['order_image'] : '../src/sample_images/product_1.jpg' ?>" class="h-56 w-full object-contain rounded-tl-2xl rounded-br-2xl mix-blend-multiply" alt="">
                                                    </div>
                                                    <div class="mt-2">
                                                        <div class="space-y-1">
                                                            <div class="text-base font-medium line-clamp-2 cursor-pointer"><?php echo isset($_COOKIE['vendor_id']) ? $row['order_title'] : 'order Name' ?></div>
                                                            <p class="space-x-2">
                                                                <span class="text-lg font-medium text-gray-500">₹<?php echo isset($_COOKIE['vendor_id']) ? $row['total_price'] : 'MRP' ?></span>
                                                            </p>
                                                            <h2>QTY: <?php echo isset($_COOKIE['vendor_id']) ? $row['qty'] : 'order Quantity' ?></h2>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="text-center divide-y-2">
                                                    <div class="flex justify-between items-center divide-x-2 border">
                                                        <div class="w-full">
                                                            <input type="text" value="<?php echo $row['order_id'] ?>" class="orderId hidden">
                                                            <button data-email="<?php echo $row['user_email'] ?>" data-id="<?php echo $i ?>" class="accept flex justify-center items-center space-x-1 w-full py-2 px-1 text-green-600">
                                                                <span>
                                                        ...
                                                        <div class="w-full">
                                                            <input type="text" value="<?php echo $row['order_id'] ?>" class="order_id hidden">
                                                            <button data-email="<?php echo $row['user_email'] ?>" data-id="<?php echo $i ?>" class="cancel flex justify-center items-center space-x-1 w-full py-2 px-1 text-red-600">
                                                                <span>
    ...
    <script>
        $(document).ready(function() {
            $('.accept').on('click', function(e) {
                ...
                let orderId = $('.orderId').eq(index).val();
                let vendorId = <?php echo $vendor_id ?>;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        loader();
                        displaySuccessMessage('Order request accept successfully')
                    }
                });
            });
            ...
            $('.cancel').click(function(e) {
                let index = $(this).data('id');
                let orderId = $('.order_id').eq(index).val();
                let vendorId = <?php echo $vendor_id ?>;
                let email = $(this).data('email');
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        loader();
                        displaySuccessMessage('Order request Cancel successfully')
                    }
                });
            });
        });
