...
$selected = isset($_SESSION['sort']) ? $_SESSION['sort'] : 'Sort';
?>
    ...
    <div class="px-3 sm:px-16 outfit mt-5" id="main-content">
            ...
            <div class="flex gap-2 relative">
                <div x-data="{ open: false, selected: '<?php echo $selected; ?>' }" class="relative inline-block text-sm text-gray-800">
                    ...
                    <div x-show="open" @keydown.escape.window="open = false" @click.outside="open = false" class="transition absolute right-0 mt-2 w-40 bg-white border-2 border-gray-300 text-gray-700 rounded-xl shadow-lg z-10 overflow-hidden text-sm divide-y-2 divide-gray-300" x-cloak>
                        ...
                        <a @click="selected = 'All'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=All`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">All</a>
                        <a @click="selected = 'Most Popular'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=Most Popular`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">Most Popular</a>
                        <a @click="selected = 'Best Rating'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=Best Rating`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">Best Rating</a>
                        <a @click="selected = 'Newest'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=Newest`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">Newest</a>
                        <a @click="selected = 'Low to High'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=Low to High`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">Low to High</a>
                        <a @click="selected = 'High to Low'; open = false; window.location.href = `?searchName=<?php echo $keywords; ?>&sort=High to Low`" class="block px-4 py-2 hover:bg-gray-200 cursor-pointer">High to Low</a>
                    </div>
                </div>
        ...
        <div class="flex jutify-center">
            <div class="mt-7 w-64 hidden lg:block">
                <form method="post" action="search_items.php?searchName=<?php echo $keywords ?>">
                    <!-- Price -->
                ...
                if ($filter_query) {
                    ...
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%') AND $filter_query ORDER BY CAST(REPLACE($sort_column, ',', '') AS UNSIGNED) $sort_order";
                } elseif ($selected === 'Newest') {
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%')";
                } elseif ($selected === 'All') {
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%')";
                } elseif ($selected === 'Low to High') {
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%') ORDER BY CAST(REPLACE($sort_column, ',', '') AS UNSIGNED) $sort_order";
                } elseif ($selected === 'High to Low') {
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%') ORDER BY CAST(REPLACE($sort_column, ',', '') AS UNSIGNED) DESC";
                    ...
                    WHERE v.vendor_id IN ($vendorIdList)
                    AND (pr.keywords LIKE '%$keywords%' OR pr.Category LIKE '%$keywords%')
                    ...
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%') ORDER BY avg_rating DESC";
                } else {
                    $vendorIdList = implode(',', $vendorIds);
                    $products = "SELECT * FROM products WHERE vendor_id IN ($vendorIdList) AND (keywords LIKE '%$keywords%' OR Category LIKE '%$keywords%' OR Type LIKE '%$keywords%' OR company_name LIKE '%$keywords%')";
                }
